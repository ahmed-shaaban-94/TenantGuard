...
- **FR-001** **[core: built / edge: deferred]**: The service MUST define a single-delivery `dispatch` that verifies the HMAC signature over the raw body before any further processing (built). Binding it to an exposed HTTP listener endpoint is the deferred edge adapter.
- **FR-002**: On a verified, reviewable event, the service MUST dispatch to the existing 014 `handleEvent` with a concrete Checks client and workspace, producing and posting a Checks run for the PR head.
- **FR-003** **[core: built / edge: deferred]**: The service MUST define a `GitHubApi` port and a `ChecksClient` adapter over it whose only writes are `checks.create` / `checks.update`, routed through the 014 safety allowlist (built). The concrete octokit-backed `GitHubApi` that calls the real Checks API is the deferred edge adapter.
- **FR-004** **[core: built / edge: deferred]**: The service MUST provide a workspace that checks out the PR head into a unique ephemeral directory and disposes of it (including self-cleanup of a partial dir on checkout failure) — built and tested against an injectable `GitRunner`. The concrete `GitRunner` that shells to real `git` is the deferred edge adapter.
...
- **FR-009**: The service MUST acknowledge a well-formed but non-reviewable event without creating a check (not treated as an error).
- **FR-010**: For a verified event the service cannot fully review (checkout failure, timeout, GitHub unavailability), it MUST conclude the check `neutral` with an honest message and MUST NOT report a false `success`.
...
- **FR-015** **[edge: port defined, adapter deferred]**: The service MUST authenticate to GitHub as the App installation by minting a per-event, short-lived installation access token, using it only for that event, and discarding it (never persisting the token or private key). The token is consumed via an injected `authToken()` port (defined); the concrete JWT-sign + installation-token exchange is the deferred edge adapter.
...
> **Verification scope:** SC-001/SC-002 describe live end-to-end operation, which depends on the deferred edge adapters (HTTP listener, octokit `GitHubApi`, real `git`). With the current delivery they are verified at the **core seam** (dispatch + handler + fakes/recording git), not yet against live GitHub. They become fully verifiable once the edge adapters are built.
